...
const Ingredient = struct { capacity: i32, durability: i32, flavor: i32, texture: i32, calories: i32 };
...
pub fn run(problem: *aoc.Problem) !aoc.Solution {
    ...
    while (problem.line()) |line| {
        ...
        _ = tokens.next().?;
        params.ingredients[ingredients_idx] = Ingredient{ .capacity = try int_from_tokens(&tokens, 1), .durability = try int_from_tokens(&tokens, 1), .flavor = try int_from_tokens(&tokens, 1), .texture = try int_from_tokens(&tokens, 1), .calories = try int_from_tokens(&tokens, 0) };
        ingredients_idx += 1;
    }
...
fn int_from_tokens(tokens: *std.mem.TokenIterator(u8, .any), offset: usize) !i32 {
    _ = tokens.next().?;
